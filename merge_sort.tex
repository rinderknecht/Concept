...
divise le problème en sous-problèmes et la synthèse combine leur
solution. Bien sûr, il faut parvenir à des sous-problèmes suffisamment
simples pour que leur solution soit évidente ou obtenue par une autre
méthode. Bien que le tri par interclassement ne soit pas difficile à
programmer, la détermination de son coût nécessite des connaissances
mathématiques approfondies. La plupart des manuels
...
\end{array}}
\end{equation*}
\caption{Interclasser deux piles triées\label{fig_mrg}
...
(équation~\eqref{def_cat} à la page~\pageref{def_cat}), nous
...
que \(\C{\fun{cat}}{m,n}\). La \fig~\vref{fig_mrg_247}
\begin{figure}
\begin{equation*}
\boxed{%
\begin{array}{r@{\;}l@{\;}l}
...
\caption{\(\fun{mrg}([3,4,7],[1,2,5,6]) \twoheadrightarrow
  [1,2,3,4,5,6,7]\)\label{fig_mrg_247}\index{tri par
    interclassement!interclassement!exemple}\index{langage
    fonctionnel!évaluation!trace}}
\end{figure}
...
\begin{equation}
\C{\fun{mrg}}{m,n} = \OC{\fun{mrg}}{m,n} + 1.\label{def_cost_mrg}
\end{equation}
...
\emph{arcs} à la \fig~\vref{fig_merged}.  Un arc entrant (côté pointu)
...
\label{fig_merged}
\end{equation}
%\caption{Interclassement de deux piles\label{fig_merged}}
%\end{figure}
...
de la \fig~\vref{fig_merged}, ce qui suggère que plus il y a de clés
...
\begin{equation}
\OB{\fun{mrg}}{m,n} = \min\{m,n\}.\label{eq_best_merge}
\end{equation}
\index{tri par interclassement!interclassement!coût minimum|)}
...
en ôtant, à la \fig~\vref{fig_merged}, les n{\oe}uds à droite
...
\begin{equation}
\OW{\fun{mrg}}{m,n} = m + n - 1.\label{eq_worst_merge}
\end{equation}
...
\(m\)~et~\(n\). Considérons la \fig~\vref{fig_mean_mrg1},
\begin{figure}[b]
\centering
...
permutations, \vpageref{par_permutations}. Déterminons donc
...
\frac{r!}{k!(r-k)!}.
\end{equation*}
Nous pouvons vérifier maintenant qu'à la \fig~\vref{fig_mean_mrg1},
...
\fig~\ref{fig_mean_mrg2}.
\begin{figure}
\centering
\includegraphics[bb=66 625 252 730]{mean_mrg2}
\caption{Dénombrement vertical\label{fig_mean_mrg2}}
\end{figure}
Cette figure a été obtenue en déplaçant la troisième colonne de la
\fig~\vref{fig_mean_mrg1} sous la deuxième colonne et en ôtant les
...
\label{eq_K}
\end{equation}
D'après les équations~\eqref{eq_KoverK} et~\eqref{eq_K}, nous tirons
\begin{equation}
K(m,n) = (m + n) \binom{m+n}{n} - W(m,n) - W(n,m).
\label{eq_K_temp}
\end{equation}
Nous pouvons décomposer \(W(m,n)\) en dénombrant \emph{verticalement}
les n{\oe}uds blancs cerclés. À la \fig~\ref{fig_mean_mrg2},
...
nommée traditionnellement \emph{le triangle de Pascal}\index{triangle
  de Pascal} et montrée à la \fig~\vref{fig_pascal_triangle}.
\begin{figure}
\centering
\includegraphics{pascal}
\caption{Le coin du triangle de Pascal (en gras)
\label{fig_pascal_triangle}}
\end{figure}
...
\fig~\vref{fig_pascal_triangle} les lignes \(r=7\) et \(r=8\):
\begin{center}
...
l'équation~\eqref{eq_K_temp} \vpageref{eq_K_temp}\index{K@$K(m,n)$}:
\begin{equation*}
K(m,n) = (m + n)
...
avec l'insertion simple à la section~\ref{sec_straight_ins}
\vpageref{sec_straight_ins}. En d'autres termes, nous devrions avoir
le théorème
\begin{equation}
...
la règle~\(\theta\) ou~\(\iota\) à la \fig~\ref{fig_mrg}, comme nous
l'avons reconnu à l'équation~\eqref{def_cost_mrg}. Alors, les
équations~\eqref{eq_ins} \vpageref{eq_ins} et~\eqref{eq_Amrg} donnent
\begin{equation*}
...
\fig~\vref{fig_mrg} est facile à établir en prenant un ordre
lexicographique \index{induction!ordre lexicographique}
(page~\pageref{par_ackermann}) sur les paires de piles qui sont, à
leur tour, partiellement ordonnées par la relation de sous-terme
propre\index{induction!ordre des sous-termes propres}
(page~\pageref{par_well-founded}), ou, plus restrictive, la
...
Pour l'instant, considérons à la \fig~\vref{fig_bot_up1}
\begin{figure}
\centering
...
équations \eqref{eq_cost_power_2} et~\eqref{eq_best_merge}, nous avons
alors
\begin{equation}
...
\B{\Join}{p}
  ...
  = p2^{p-1}.\label{eq_best_power}\index{mrg@$\B{\Join}{n}$}
\end{equation}
\index{tri par interclassement!puissance@$2^n$ clés!coût minimum|)}
...
\eqref{eq_cost_power_2}~et~\eqref{eq_worst_merge},
\begin{equation}
%\abovedisplayskip=2pt
%\belowdisplayskip=0pt
\W{\Join}{p}
  ...
  = (p-1)2^p + 1.
\label{eq_worst_power}\index{mrg@$\W{\Join}{n}$}
\end{equation}
\index{tri par interclassement!puissance@$2^n$ clés!coût maximum|)}
...
donnée. Donc l'équation~\eqref{eq_cost_power_2} est satisfaite par
...
équations~\eqref{eq_Amrg_n_n} et~\eqref{def_cost_mrg} donnent
\begin{equation*}
%\abovedisplayskip=2pt
...
\end{equation*}
Avec l'équation \eqref{eq_cost_power_2}, nous obtenons de plus, pour
\(p > 0\),
\begin{align}
%\abovedisplayskip=0pt
%\belowdisplayskip=0pt
...
réalisant ce tri est montré à la \fig~\vref{fig_tms}\index{tri par
...
\end{array}}
\end{equation*}
\caption{Tri par interclassement descendant avec \fun{tms/1}\label{fig_tms}}
\end{figure}
...
\caption{\label{fig_bits}}
%\caption{Binary numbers from \(1\) to \(n\)\label{fig_bits}}
\end{wrapfigure}
Nous avons \(\OB{\fun{tms}}{n} = \OB{\fun{tms}}{\floor{n/2}} +
\OB{\fun{tms}}{\ceiling{n/2}} +
\floor{n/2}\),\index{tms@$\OB{\fun{tms}}{n}$} via
l'équation~\eqref{eq_best_merge} \vpageref{eq_best_merge}.
En particulier,
\begin{equation*}
...
l'équation~\eqref{eq_ruler_nu} \vpageref{eq_ruler_nu}. Définissons-là
récursivement:
\begin{equation}
\nu_{0} := 0,\quad \nu_{2n} := \nu_{n},\quad
\nu_{2n+1} := \nu_{n} + 1.\label{def_nu}\index{somme des bits}
\end{equation}
...
l'équation~\eqref{eq_best_power}, nous avons \(\OB{\fun{tms}}{2^p} =
...
a(m+i) + b \leqslant
\OB{\fun{tms}}{m+i}\). Avec~\eqref{ineq_Btms_n_i}, cela est impliqué
par
\begin{equation*}
...
\tfrac{1}{2}m\lg m + (\tfrac{1}{2}i\lg i + ai + b) + i.
\end{equation*}
À ce point, nous plongeons~\(i\) dans les nombres réels en posant
\(i=x2^p\), où \(x\)~est un nombre réel tel que \(0 < x \leqslant
1\). Alors, l'inégalité suivante implique le résultat:
\begin{equation*}
a \leqslant \Phi(x), \;\text{où \(\Phi(x) := \tfrac{1}{2}x\lg x - \tfrac{1}{2}(1+x)\lg(1+x) + x\)}.
\end{equation*}
La fonction~\(\Phi\) est continûment prolongée en~\(0\) car \(\lim_{x
  \to 0} x\lg x = 0\), et elle est dérivable sur l'intervalle fermé
\([0,1]\):
\begin{equation}
...
\begin{itemize}
  ...
  entier: \(2^k\!/3 = \floor{2^k\!/3} + 1/3\) et \(\floor{2^k\!/3} =
  (4^m-1)/3 = 4^{m-1} + 4^{m-2} + \dots + 1 = 2^{2m-2} + 2^{2m-4} +
  \dots + 1 = (1010\dots01)_2\). Donc \(\nu_{\phi_{2m}} = m\). On a
  \(\OB{\fun{tms}}{m+1} = \OB{\fun{tms}}{m} + \nu_m\), donc
  ...
  \((4^m-1)/3\) est un entier, nous avons donc \(2^k\!/3 = \floor{2^k\!/3}
  + 2/3 = \ceiling{2^k\!/3} - 1/3\) et \(\floor{2^k\!/3} = 2(4^m-1)/3
  = 2^{2m-1} + 2^{2m-3} + \dots + 2 = (1010\dots10)_2\); d'où
  \(\nu_{\phi_{2m+1}} = m\). D'après \(\OB{\fun{tms}}{m+1} =
  \OB{\fun{tms}}{m} + \nu_m\) vient \(\OB{\fun{tms}}{1+\phi_{2m+1}} -
  \OB{\fun{tms}}{\phi_{2m+1}} = m\), ou, de manière
  équivalente,\index{tms@$\OB{\fun{tms}}{n}$|(}
...
\begin{itemize}
  ...
  l'équation~\eqref{eq_OBtms_2m_i}, \(\OB{\fun{tms}}{\phi_{2m+2}} =
  \OB{\fun{tms}}{2^{2m}} + \smash[t]{\OB{\fun{tms}}{\phi_{2m}}} +
  \phi_{2m} = \smash[t]{\OB{\fun{tms}}{\phi_{2m}}} - m4^m +
  \phi_{2m}\). En ajoutant membre à membre pour des valeurs
  croissantes de \(m=0\) à \(m=n-1\) entraîne
  \(\OB{\fun{tms}}{\phi_{2n}} = \OB{\fun{tms}}{\phi_0} + S_n +
  \sum_{m=0}^{n-1}{\phi_{2m}}\), où \(S_n :=
  \sum_{m=0}^{n-1}{m4^m}\). Nous avons \(S_n + n4^n =
  \sum_{m=1}^{n}{m4^m} = \sum_{m=0}^{n-1}(m+1)4^{m+1} = 4 \cdot S_n +
  4\sum_{m=0}^{n-1}4^m\). Finalement, nous tirons \(9 \cdot S_n = (3n
  -4)4^n + 4\). D'un autre côté, \(9 \sum_{m=0}^{n-1}{\phi_{2m}} = 4^n
  - 3n - 1\). Enfin, en remarquant que \(\phi_0 = 0\) et
  \(\OB{\fun{tms}}{0} = 0\), nous obtenons
  \begin{equation}
      \OB{\fun{tms}}{\phi_{2n}} = (n-1)\phi_{2n}.
    \label{eq_OBtms_phi_2n}
  \end{equation}
  Cherchons maintenant \(Q(\phi_{2n}) = \tfrac{1}{2}\phi_{2n}(2(n-1) +
  \lg(1-1/4^n)) = \OB{\fun{tms}}{\phi_{2n}} +
  \tfrac{1}{2}\phi_{2n}\lg(1-1/4^n)\), en usant
  de~\eqref{eq_OBtms_phi_2n}. Si nous définissons \(f(x) := (1-x)
  \ln(1-1/x)\), alors \(\OB{\fun{tms}}{\phi_{2n}} - Q(\phi_{2n}) =
  f(4^n)/(6\ln 2)\). Un peu d'analyse réelle élémentaire révèle que
  \(3\ln\tfrac{3}{4} \leqslant f(x) < 1\), avec \(x \geqslant 4\),
  c'est-à-dire \(1 - \tfrac{1}{2}\lg 3 \leqslant
  \OB{\fun{tms}}{\phi_{2n}} - Q(\phi_{2n}) < 1/(6\ln 2)\), si \(n
  \geqslant 1\). Cela donne l'encadrement \(\boxed{0.2075 <
  \OB{\fun{tms}}{\phi_{2n}} - Q(\phi_{2n}) < 0.2405.}\)

  D'après le théorème~\ref{thm_OB_lambda} et
  l'équation~\eqref{eq_OBtms_phi_2n}, nous avons
  \begin{equation}
    \OB{\fun{tms}}{1+\phi_{2n}} = (n-1)(1+\phi_{2n}) + 1.
      \label{eq_OBtms_succ_phi_2n}
  \end{equation}
  De plus, \(Q(1+\phi_{2n}) = \tfrac{1}{2}(1+\phi_{2n}) (2(n-1) +
  \lg(1+1/2^{2n-1})) = \OB{\fun{tms}}{1+\phi_{2n}} - 1 +
  \tfrac{1}{6}(4^n + 2)\lg(1+2/4^n)\), la dernière égalité reposant
  sur l'égalité~\eqref{eq_OBtms_succ_phi_2n}. Si \(g(x) := 1 -
  \tfrac{1}{6}(x+2)\lg(1+2/4^n)\), alors \(\OB{\fun{tms}}{1+\phi_{2n}}
  - Q(1+\phi_{2n}) = g(4^n)\). Une analyse élémentaire montre que \(2
  - \lg 3 \leqslant g(x) < 1 - 1/(3\ln 2)\), quand \(x \geqslant 4\),
  c'est-à-dire \(2 - \lg 3 \leqslant g(4^n) < 1 - 1/(3\ln 2)\), où \(n
  \geqslant 1\). D'où l'encadrement \(\boxed{0.4150 <
  \OB{\fun{tms}}{1+\phi_{2n}} - Q(1+\phi_{2n}) < 0.5192.}\)
...
\item Si \(k=2m+1\), alors \(\phi_{2m+1} = 2^{2m-1} + \phi_{2m-1}\)
  (théorème~\ref{thm_OB_lambda}). D'après
  l'équation~\eqref{eq_OBtms_2m_i}, nous tirons
  \(\OB{\fun{tms}}{\phi_{2m+1}} = \OB{\fun{tms}}{2^{2m-1}} +
  \smash[t]{\OB{\fun{tms}}{\phi_{2m-1}}} + \phi_{2m-1} =
  \smash[t]{\OB{\fun{tms}}{\phi_{2m-1}}} - (2m-1)4^{m-1} +
  \smash[t]{\phi_{2m-1}}\). En sommant membre à membre pour des valeurs
  croissantes de \(m=1\) à \(m=n-1\), et en multipliant par~\(9\)
  entraîne \(9\OB{\fun{tms}}{\phi_{2n+1}} = \tfrac{1}{2}S_{n+1} -
  3(4^n-1) + \sum_{m=0}^{n-1}\phi_{2m+1}\), qui se simplifie en
  \begin{equation}
      \OB{\fun{tms}}{\phi_{2n+1}} = (n - \tfrac{1}{2})\phi_{2n+1}.
      \label{eq_OBtms_phi_2n_1}
  \end{equation}
  Nous avons alors \(Q(\phi_{2n+1}) = \tfrac{1}{2}\phi_{2n+1}(2n-1 +
  \lg(1-1/4^n)) = \OB{\fun{tms}}{\phi_{2n+1}} +
  \tfrac{1}{2}\phi_{2n+1}\lg(1-1/4^n)\), où la dernière égalité
  résulte de~\eqref{eq_OBtms_phi_2n_1}. Si nous posons \(f(x) :=
  (1-x)\ln(1-1/x)\), comme nous l'avons fait pour \(Q(\phi_{2n})\),
  nous avons \(\OB{\fun{tms}}{\phi_{2n+1}} - Q(\phi_{2n+1}) =
  f(4^n)/(3\ln 2)\). Nous savons déjà que \(3\ln\tfrac{3}{4} \leqslant
  f(x) < 1\), si \(x \geqslant 4\), c'est-à-dire \(\lg 3 - 2 \leqslant
  \OB{\fun{tms}}{\phi_{2n+1}} - Q(\phi_{2n+1}) < 1/(3\ln 2)\), si \(n
  \geqslant 1\). Nous avons donc l'encadrement \(\boxed{0.4150 <
  \OB{\fun{tms}}{\phi_{2n+1}} - Q(\phi_{2n+1}) < 0.4809.}\)

  D'après le théorème~\ref{thm_OB_lambda} et
  l'équation~\eqref{eq_OBtms_phi_2n_1}, nous déduisons
  \begin{equation}
    \OB{\fun{tms}}{1+\phi_{2n+1}} = (n-\tfrac{1}{2})(1+\phi_{2n+1})
    + \tfrac{1}{2}.
      \label{eq_OBtms_succ_phi_2n_1}
  \end{equation}
  De plus, \(Q(1+\phi_{2n+1}) = \tfrac{1}{2}(1+\phi_{2n+1})(2n-1 +
  \lg(1+1/2^{2n+1})) = \OB{\fun{tms}}{1+\phi_{2n+1}} - \tfrac{1}{2} +
  \tfrac{1}{6}(1+2^{2n+1})\lg(1+1/2^{2n+1})\), la dernière égalité
  résultant de~\eqref{eq_OBtms_succ_phi_2n_1}. Si nous posons \(h(x)
...
de~\eqref{ineq_lower_Btms} est atteint si \(n=2\) (le cas de base) et
...
(voir~\eqref{eq_der_Phi}). D'où: \(a'_{\min} := \max_{0 \leqslant x
...
\tfrac{1}{2}n\lg n.
\label{ineq_bounds_Btms}
\index{tms@$\OB{\fun{tms}}{n}$}
\end{equation}
Le majorant est clairement atteint si \(n=2^p\) à cause de
l'équation~\eqref{eq_best_power}. Il est aussi évident maintenant que
...
dénombrement de bits de la \fig~\vref{fig_bits}. Rappelons que
...
\index{mrg@\fun{mrg/2}}
\end{equation*}
L'équation~\eqref{eq_worst_merge} \vpageref{eq_worst_merge} done
\(\OW{\fun{tms}}{n} = \OW{\fun{tms}}{\floor{n/2}} +
...
l'équation~\eqref{eq_e_r}, donc \(\Delta_n = \floor{\lg n} +
...
la figure \fig~\vref{fig_bits}. La plus grande puissance de~\(2\) qui
...
\ceiling{\lg(n+1)}\), ce qui, avec l'aide de l'équation~\eqref{eq_e_r}
\vpageref{eq_e_r}, prouve \(1 + \floor{\lg n} =
\ceiling{\lg(n+1)}\). En conséquence, l'équation~\eqref{eq_tms_n_tmp}
...
\begin{itemize}
  ...
  \item sinon, \(\ceiling{\lg n} = \floor{\lg n} + 1 = \lg n - \{\lg
    ...
    \floor{x}\) est la \emph{partie fractionnaire}\index{partie
      fractionnaire@$\{x\}$|see{partie fractionnaire}}\index{partie
      fractionnaire} du réel~\(x\). En particulier, \(0 \leqslant
...
n\lg n - 0.91 n + 1.\label{ineq_OWtms}
\end{equation}
...
probables, l'équation~\eqref{eq_cost_tms} devient
\begin{equation*}
...
\end{equation*} 
L'équation~\eqref{eq_Amrg} implique, à son tour,
\begin{equation*}
\OM{\fun{tms}}{n} = \OM{\fun{tms}}{\floor{n/2}} +
...
\(\OB{\fun{tms}}{n}\). Les inéquations~\eqref{ineq_M_join}
\vpageref{ineq_M_join} sont équivalentes à \(n\lg n - \alpha n +
...
l'inéquation~\eqref{ineq_base_lower_Atms} nous dit que nous devons
...
\(\pred{H}{n}\) dans~\eqref{ineq_base_lower_Atms} est ici inversé:
...
La \fig~\vref{fig_bot_up2}
\begin{figure}
\centering
\subfloat[Arbre pour \({[}7,3,5,1,6,2,4{]}\)\label{fig_bot_up2}]{%
\includegraphics[bb=58 632 192 721]{bot_up2}}
\qquad
\subfloat[Longueurs seules\label{fig_msort_abs}]{%
...
comme on peut en voir un exemple à la \fig~\vref{fig_msort_abs}.
...
avec \(0 < i < 2^p\). Alors, de l'équation~\eqref{eq_cost_bms}
\vpageref{eq_cost_bms}, et~\eqref{eq_best_merge}
\vpageref{eq_best_merge}, nous déduisons:
\begin{equation*}
...
l'équation~\eqref{eq_OBtms_2m_i} \vpageref{eq_OBtms_2m_i}. Par
conséquent,
\begin{equation}
...
lorsque \(n\)~est un nombre de Jacobsthal\index{Jacobsthal!nombre de
  $\sim$} (voir équations~\eqref{eq_Jacobsthal}
\vpageref{eq_Jacobsthal}). Le majorant est atteint quand \(n=2^p\).
\index{bms@$\OB{\fun{bms}}{n}$|)}
\index{tri par interclassement!$\sim$ ascendant!coût minimum|)}
...
avec \(0 < i < 2^p\). Alors, de l'équation~\eqref{eq_cost_bms}
\vpageref{eq_cost_bms}, et~\eqref{eq_worst_merge}
\vpageref{eq_worst_merge}, nous déduisons
\begin{equation}
...
l'équation~\eqref{eq_worst_power} \vpageref{eq_worst_power}, qui prend
ici la forme \(\OW{\fun{bms}}{2^p} = p2^p - 2^p +
1\). Alors~\eqref{eq_Wbms_2p_i} entraîne:
\begin{equation*}
...
cette décomposition à l'équation~\eqref{eq_e_r} à la
page~\pageref{eq_e_r}.
...
\label{fig_msort_gen}}
\end{wrapfigure}
Considérons alors à la \fig~\ref{fig_msort_gen} l'arbre de tous les
...
\begin{itemize}
  ...
  \item \emph{Si \(n\)~est pair}, à la \fig~\vref{fig_w2p}, cette somme
  est \(\OW{\fun{bms}}{n} - r\). Elle
  égale \(\OW{\fun{bms}}{n+1} - 2^{e_0} - \OW{\fun{bms}}{2^0}\) à la
  \fig~\vref{fig_w2p_succ}. En égalant les deux:
  \begin{equation}
  ...
  l'équation~\eqref{eq_OW1} implique alors la récurrence
    \begin{equation}
      \OW{\fun{bms}}{2k+1} = \OW{\fun{bms}}{2k} + 2^{\rho_{2k}} +
      \nu_{2k} - 1.
      \label{eq_OWbms_2k_1_tmp}
    \end{equation}
    ...
    l'équation~\eqref{eq_OWbms_2k_1_tmp} en
    \begin{equation}
...
\label{fig_Wn_odd}}
\end{figure}
La somme des expressions non-encadrées à la \fig~\ref{fig_w2p1} est
...
l'équation~\eqref{eq_worst_power} et \(\sum_{k=0}^{q-1}k2^k =
...
d'où
     \begin{equation}
       \abovedisplayskip=4pt
       \belowdisplayskip=4pt
       S_{q} = \smash[t]{\textstyle\sum_{k=1}^{q-1}{k2^{k-1}}} = (q-2)2^{q-1} + 1.\label{eq_Sj}
     \end{equation}
     La même somme à la \fig~\ref{fig_w2p1_succ}
     ...
     \begin{equation*}
       ...
       \OW{\fun{bms}}{n} + \nu_n.
     \end{equation*}
     En nous souvenant des récurrences~\eqref{def_nu} \vpageref{def_nu} et
...
\end{itemize}
D'après les équations~\eqref{eq_OWbms_2k} et~\eqref{eq_OWbms_2k_1},
nous déduisons
\begin{equation*}
...
l'équation~\eqref{eq_OBbms}.  \index{bms@$\OW{\fun{bms}}{n}$|)}
\index{tri par interclassement!$\sim$ ascendant!coût maximum|)}
...
étant également probables, l'équation~\eqref{eq_cost_bms}
\vpageref{eq_cost_bms} donne \(\OM{\fun{bms}}{0} = \OM{\fun{bms}}{1}
...
\end{equation*}
qui, avec l'équation~\eqref{eq_Amrg}, à son tour implique
...
\(\OB{\fun{tms}}{n}\) dans l'inégalité~\eqref{ineq_McIlroy}
\vpageref{ineq_McIlroy}. Commençons avec le minorant et cherchons à
...
emploierons la valeur exacte à l'équation~\eqref{eq_Mjoin}
\vpageref{eq_Mjoin}, où \(\alpha := \sum_{k \geqslant
  0}1/(2^k+1)\). D'après l'équation~\eqref{eq_Abms_2p_i}, nous
dérivons
\begin{equation*}
...
l'inéquation~\eqref{ineq_base_lower_Btms}, nous tirons \(b_{\max} =
...
interclassement descendant, à l'inégalité~\eqref{ineq_lower_Atms}
\vpageref{ineq_lower_Atms}, parce que là-bas, nous avions
...
que~\eqref{ineq_M_join} \vpageref{ineq_M_join}, ce qui est une bonne
surprise.
...
majorant \(c_p < 2\) des inégalités~\eqref{ineq_M_join}
\vpageref{ineq_M_join} et en définissant
\begin{equation*}
...
\leqslant a'\) et croisons nos doigts. Dans la \fig~\vref{fig_phi},
\begin{figure}
\centering
\includegraphics[bb=71 565 400 725]{phi} 
\caption{\(\Phi(p,1)\), \(\Phi(p,2)\) et \(\Phi(p,3)\)\label{fig_phi}}
\end{figure}
...
de croître. (Voir à nouveau la \fig~\vref{fig_phi}.)
...
\fig~\vref{fig_phi}. Nous avons \(i/(i+1) - \alpha \to 1 - \alpha\),
...
\end{array}}
\end{equation*}
\caption{Tri par interclassement ascendant avec \fun{bms/1}\label{fig_bms}}
\end{figure}
...
d'interclassement. Nous montrons à la \fig~\vref{fig_bms} la
...
\item Les règles \(\theta\)~et~\(\iota\) sont utilisées une fois pour
  ...
  \fig~\vref{fig_msort_gen} révèle
    ...
    \(\OC{\fun{mrg}}{m,n} = 1\) à l'équation~\eqref{eq_cost_power_2}
    \vpageref{eq_cost_power_2}, donnant \(\OC{\fun{bms}}{2^p} = 2^p -
    1\). En substituant ce résultat dans
    l'équation~\eqref{eq_msort_gen}, nous tirons \(\OC{\fun{bms}}{n} =
    n - 1\). En d'autres termes, les règles \(\theta\)~et~\(\iota\)
    sont employées \(n-1\)\label{eq_bms_merges} fois au
...
\begin{proof}
 ...
 \end{proof}
    ...
    \end{equation*}
    Ceci et l'équation~\eqref{eq_n_on_power_2} impliquent que
    \begin{equation*}
    ...
    \(n=2^{m-1}\), et \(z=m\) sinon. L'équation~\eqref{eq_e_r}
    \vpageref{eq_e_r} stipule \(m = \floor{\lg n} + 1\), donc
...
\fig~\vref{fig_bms0}, où \fun{solo/1}\index{solo@\fun{solo/1}} a
...
\end{array}}
\end{equation*}
\caption{Tri ascendant accéléré avec \fun{bms\(_0\)/1}\label{fig_bms0}}
\end{figure}
...
(voir~\eqref{eq_Jacobsthal} \vpageref{eq_Jacobsthal}). Le majorant est
...
\label{ineq_topbot}
\end{equation}
Par~\eqref{eq_2p}, il est clair que \(\pred{W\(_L\)}{2^0}\) est
...
\noindent Des équations \eqref{eq_bot} \& \eqref{eq_2p}
...
= \floor{\lg n} + 1\). L'équation~\eqref{eq_top} \vpageref{eq_top}
...
l'équation~\eqref{eq_top}, cette inégalité équivaut à
\begin{equation}
  \abovedisplayskip=2pt
  \belowdisplayskip=2pt
...
\begin{itemize}
  ...
  \item Si \(i=2^q\), alors l'équation~\eqref{eq_2p} implique
...
Si \(n=2^p+1\), alors \(p=\floor{\lg n}=\ceiling{\lg
  n}-1\). Par ailleurs, la définition~\eqref{eq_bot} implique
\(\OW{\fun{bms}}{2^p+1} = p2^p+1\) et la définition~\eqref{eq_top}
\vpageref{eq_top} \(\OW{\fun{tms}}{2^p+1} = (p-1)2^p+p+2\), donc
...
\ceiling{\lg n} - 1.
\label{ineq_upper_Wbms}
\index{WU@$\predName{W}_U$}
\end{equation}
Notons comment l'équation~\eqref{eq_2p} implique
...
Soit \(n=2^p+i\). De \eqref{eq_bot} \& \eqref{eq_2p} vient
...
\item \(i = 2^q\), avec \(0 \leqslant q < p\). Alors \(\lg i = q\) et
  l'inéquation~\eqref{eq_conj2} équivaut à \((p-q+1)(2^q-1) \geqslant
  ...
  \item \(i = 2^q + j\), avec \(0 \leqslant q < p\) et \(0 < j <
    ...
    1\) et l'inéquation~\eqref{eq_conj2} est donc équivalente à \(1
    \leqslant (2^q + j - 1) (p-q) + 2^q\), ou
    \begin{equation}
     1 \leqslant (p-q+1)2^q + (p-q-1)(j-1).\label{ineq_2q_j}
    \end{equation}
    ...
    \((p-q-1)(j-1) \geqslant 0\), donc l'inéquation~\eqref{ineq_2q_j}
...
partie~\ref{part_implementation}, mais voici déjà comment calculer
...
le cas le plus défavorable (voir~\eqref{ineq_topbot}
\vpageref{ineq_topbot}), et que leur coût minimum sont égaux
(voir~\eqref{eq_OBbms} \vpageref{eq_OBbms}), ceci scelle le sort de la
variante ascendante, qui renaîtra sous une forme plus utile à la
section~\vref{sec_online}. Nous voulons prouver par induction:
\begin{equation*}
...
de l'équation~\eqref{eq_Abms_2p_i} \vpageref{eq_Abms_2p_i}:
\begin{equation*}
\OM{\fun{bms}}{2^p+i} = \OM{\fun{bms}}{2^p} + \OM{\fun{bms}}{i}
+ 2^p + i - \frac{2^p}{i+1} - \frac{i}{2^p+1}.
\end{equation*}
...
un ordre lexicographique (voir définition~\eqref{def_lexico}
\vpageref{def_lexico}). Le cas de base, \((0,0)\), est aisément
...
à~\eqref{ineq_Atms_Abms}, nous prouvons le théorème
...
\(n=m+1\). Pour~\eqref{ineq_Atms_Abms}, ceci signifie \(i=2^p\) ou
\(i=2^p-1\), soit
\begin{equation*}
...
l'équation~\eqref{eq_ave_i2wb} \vpageref{eq_ave_i2wb} le coût moyen du
...
obtenu l'encadrement~\eqref{ineq_bounds_Mbms}
\vpageref{ineq_bounds_Mbms} de \(\OM{\fun{bms}}{n}\), le majorant
...
Plus précisément, considérons à nouveau la \fig~\ref{fig_msort_gen}
\vpageref{fig_msort_gen} sans les interclassements
...
Notre programme est montré à la \fig~\ref{fig_oms}\index{tri par
...
\end{array}}
\end{equation*}
\caption{Tri par interclassement en ligne avec \fun{oms/1}\label{fig_oms}}
\end{figure}
...
\fig~\vref{fig_msort_gen}. Par exemple, la configuration de la pile
...
Notons que le programme à la \fig~\vref{fig_oms} ne capture pas
...
avons déjà rencontré à la section~\ref{sec_queueing}), suivie d'une
...
définition~\eqref{eq_C_unbal} page~\pageref{eq_C_unbal} nous rappelle
que
\begin{equation}
\OC{\fun{unb}}{n}
= \OC{\ltimes}{n}
...
l'équation~\eqref{eq_sum_2p}, nous obtenons les équations pour le
...
récurrences~\eqref{eq_ruler} \vpageref{eq_ruler} sur la
...
à la définition de~\(\OB{\fun{sum}}{2p}\) dans~\eqref{eq_B_oplus}
produit
\begin{equation}
2p\lg p - p + 4 < \OB{\fun{sum}}{2p} \leqslant 2p\lg p + 3p.
\label{ineq_B_oplus_2p}
\end{equation}
...
\noindent Grâce à~\eqref{eq_best_merge} \&~\eqref{eq_cost_unb} on a
...
\label{ineq_OBunb}
\end{equation}
Trivialement, \(0 < \OB{\fun{unb}}{n}\), donc~\eqref{eq_ocost_online}
...
l'équation~\eqref{eq_sum_2p}, nous obtenons des équations pour le
nombre maximum de comparaisons, que nous pouvons simplifier avec
l'aide de l'équation~\eqref{eq_worst_merge} \vpageref{eq_worst_merge}
en
\begin{equation}
\OW{\fun{sum}}{2p}
  = \OW{\fun{sum}}{2p+1}
...
\nu_p\) \index{somme des bits|)} à l'équation~\eqref{eq_ruler_nu},
\vpageref{eq_ruler_nu}. Nous pouvons maintenant
simplifier~\eqref{eq_OW_sum_2p} ainsi:
\begin{equation*}
\OW{\fun{sum}}{2p}
 = \OW{\fun{sum}}{2p+1}
...
l'inégalité~\eqref{ineq_B_oplus_2p} nous amène à \(\OW{\fun{sum}}{2p}
= \OW{\fun{sum}}{2p+1} \sim 4p\lg p\).

Les équations~\eqref{eq_worst_merge} et~\eqref{eq_cost_unb}, ainsi que
l'inéquation~\eqref{ineq_OBunb} impliquent
\begin{equation*}
...
\begin{itemize}
  ...
  \item Les règles \(\omega\), \(\gamma\) et~\(\delta\) sont employées
    ...
    l'équation~\eqref{eq_ruler_nu}. Nous devons aussi tenir compte des
...
2\). L'équation~\eqref{eq_OBbms} \vpageref{eq_OBbms} implique
\(\C{\fun{oms}}{n} = \OC{\fun{oms}}{n} + 3n + \floor{\lg n} +
\OB{\fun{bms}}{n} + 2\). L'encadrement~\eqref{ineq_bounds_Btms}
\vpageref{ineq_bounds_Btms} implique \(\OB{\fun{bms}}{n} \sim
...
\begin{enumerate}
  ...
  \item Trouvez \(\C{\fun{tms}}{n} - \OC{\fun{tms}}{n}\). \emph{Aide:}
    pensez à l'équation~\eqref{eq_ruler_nu} \vpageref{eq_ruler_nu}.

  \item À la page~\pageref{eq_bms_merges}, nous avons trouvé que le
    ...
    l'équation~\eqref{eq_cost_tms} \vpageref{eq_cost_tms}.)

  \item Trouvez un dénombrement d'éléments de la table à la
    \fig~\vref{fig_bits} montrant que
    \begin{equation*}
