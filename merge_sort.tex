...
a(m+i) + b \leqslant
\OB{\fun{tms}}{m+i}\). Avec~\eqref{ineq:Btms_n_i}, cela est impliqué
par
\begin{equation*}
...
\begin{itemize}
  ...
  l'équation~\eqref{eq:OBtms_2m_i}, \(\OB{\fun{tms}}{\phi_{2m+2}} =
  ...
  de~\eqref{eq:OBtms_phi_2n}. Si nous définissons \(f(x) := (1-x)
  ...
  sur l'égalité~\eqref{eq:OBtms_succ_phi_2n}. Si \(g(x) := 1 -
...
\item Si \(k=2m+1\), alors \(\phi_{2m+1} = 2^{2m-1} + \phi_{2m-1}\)
  ...
  résulte de~\eqref{eq:OBtms_phi_2n_1}. Si nous posons \(f(x) :=
  ...
  résultant de~\eqref{eq:OBtms_succ_phi_2n_1}. Si nous posons \(h(x)
...
\vpageref{eq:e_r}, prouve \(1 + \floor{\lg n} =
