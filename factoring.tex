...
À la section~\ref{def_linear_search}, \vpageref{def_linear_search},
...
\fun{pre/5}\index{pre@\fun{pre/5}} à\index{recherche de motifs!$\sim$
  na\"{\i}ve!programme} la \fig~\vref{fig_loc}. Cette sorte de
...
\vpageref{def_lexico}), avec \(s \succ t\) si~\(t\) est la sous-pile
...
\vpageref{eq_Border} qui définissent le bord maximum peuvent être
...
par l'équation~\eqref{eq_MP_failure} \vpageref{eq_MP_failure}, nous
déduisons
\begin{equation}
...
définition~\eqref{eq_Border} \vpageref{eq_Border}: \({\Border{}{a} :=
...
\fun{fail/2}\index{fail@\fun{fail/2}} à la \fig~\ref{fig_fail}.
\begin{figure}[b]
\centering
\abovedisplayskip=0pt
\belowdisplayskip=0pt
...
(définition~\eqref{def_rev} \vpageref{def_rev}), et
...
\vpageref{eq_Border}, la détermination du bord maximum d'un mot
...
\fig~\ref{fig_loc} \vpageref{fig_loc}, tout en gardant un {\oe}il sur
...
L'exemple \word{abacabac} est montré à la \fig~\ref{fig_metaprog}. Les
...
même rôle que dans la \fig~\ref{fig_mp} \vpageref{fig_mp}. La première
...
Par exemple, l'automate de la \fig~\vref{fig_abacabac} est amélioré de
...
\begin{enumerate}
  ...
  \item Proposez une simple modification pour éviter l'appel à
    \fun{rev/1} à la \fig~\vref{fig_pp}.\label{factoring_trick}
