...
\fig~\vref{fig_bullets}
\begin{figure}
\centering
\includegraphics{bullets}
\caption{Les paires critiques sont toutes joignables\label{fig_bullets}}
\end{figure}
...
\fig~\vref{fig_fib5shape}
\begin{figure}[b]
\centering
\includegraphics{fib5shape}
\caption{Forme d'un arbre\label{fig_fib5shape}}
\end{figure}
...
la \fig~\vref{fig_tree} a pour racine~\fun{f} et pour feuilles~\(0\),
...
\(\fun{e}([1])\). La \fig~\ref{fig_tree} représente l'arbre associé à
...
ne le soit. Cette stratégie est dite d'\emph{appel par
  ...
  valeurs}.\label{def_call-by-value} Malheureusement, elle conduit à
...
\begin{itemize}
  ...
  \item \(\fun{nil}() \in \mathcal{S}\);\label{def_stack}
...
Posons les abréviations commodes \(\el :=
\fun{nil}()\)\index{nil@\fun{nil/0}|)} et \(\cons{x}{s} :=
\fun{cons}(x,s)\)\index{cons@\fun{cons/2}|)}, d'après la convention du
langage de programmation \Prolog
\citep{SterlingShapiro_1994,Bratko_2000}.  Par exemple, nous pourrions
écrire \([1|[2|[3|\el]]]\) au lieu de
\(\fun{cons}(1,\fun{cons}(2,\fun{cons}(3,\fun{nil}())))\). Nous
pouvons de plus abréger la notation \(\cons{x_1}{\cons{x_2}{\dots
    \cons{x_n}{s}}}\) en \(\cons{x_1,x_2,\dots,x_n}{s}\), ainsi que
\(\cons{x}{\el}\) en \([x]\). Par exemple, \([1|[2|[3|\el]]]\) est
plus succinctement décrite par \([1,2,3]\). Notre système définissant
...
\cons{x}{\fun{cat}(s,t)}.
\label{def_cat}\index{pile!concaténation!définition}
\end{equation}
...
ordinateur. Considérons par exemple dans la \fig~\vref{fig_cat_dag} la
même définition de \fun{cat/2} telle que donnée
dans~\eqref{def_cat}. Les flèches en lieu d'arcs dénotent un partage
...
définition~\eqref{def_cat}\index{cat@\fun{cat/2}}:
\begin{equation*}
\fun{cat}(        \el,t) \xrightarrow{\smash{\alpha}} t;\qquad
\fun{cat}(\cons{x}{s},t) \xrightarrow{\smash{\beta}}
\cons{x}{\fun{cat}(s,t)}.
...
de~\(s\). La règle~\(\alpha\) nous amène à l'équation
\(\C{\fun{cat}}{0} \eqn{\smash{\alpha}} 1\) et la règle~\(\beta\) à
\(\C{\fun{cat}}{n+1} \eqn{\smash{\beta}} 1 + \C{\fun{cat}}{n}\);
prises ensemble, elles conduisent à \(\C{\fun{cat}}{n} = n +
1\).\label{cost_cat}\index{pile!concaténation!coût}
...
de~\(n\) est trouvé en posant \(n := \sum_{i=0}^{m-1}{b_i2^i}\), où les~\(b_i\)
sont les bits et \(b_{m-1}=1\). Alors
\begin{equation}
...
\fig~\vref{fig_flips}, où les bits qui vont basculer au prochain
incrément sont en gras.
\begin{figure}[t]
\centering
\subfloat[Basculements de bits\label{fig_flips}]{%
\includegraphics[bb=68 463 168 721]{flips}}
\qquad
\subfloat[$F(n) = \sum_{i \geqslant 0}\floor{n/2^i}$, où $n=22$\label{fig_ruler}]%
{\includegraphics{ruler}}
\caption{Bits comptés verticalement et en diagonale}
\end{figure}
...
\fig~\vref{fig_ruler}, où, \emph{en diagonale} les bits à~\(1\) à la
...
\emph{somme des bits}\index{somme des bits} ou \emph{poids de
  Hamming}; par exemple, dans la \fig~\vref{fig_ruler}, nous pouvons
...
\end{equation*}
Deux énumérations, l'une verticale, l'autre en diagonale, ont montré
que le nombre total exact de basculements de bits est d'un ordre de
...
\citep{Stanley_1997,Stanley_1999,Martin_2001} pour atteindre son but;
...
ainsi: \(a \equiv b\) s'il existe une valeur~\(v\) telle que \(a
\xrightarrow{\smash[t]{*}} v\) et \(b \xrightarrow{\smash{*}} v\). Par
exemple, \(\fun{cat}([1,2], [3,4]) \equiv \fun{cat}([1],
...
\equiv y\). Si nous voulons prouver des équivalences avec des
variables prenant leur valeurs dans des ensembles infinis, comme
\(\fun{cat}(s,\fun{cat}(t,u)) \equiv
\fun{cat}(\fun{cat}(s,t),u)\)\index{cat@\fun{cat/2}|)}, nous avons
besoin d'un principe d'induction.

\mypar{Induction bien fondée}
\label{par_well-founded}
...
relations \(a_0 \succ a_1 \succ \dots\) Le \emph{principe d'induction
  bien fondée} énonce alors que, pour tout prédicat~\(\aleph\), sa
validité universelle \(\forall a \in A.\aleph(a)\) est impliquée par
\(\forall a.(\forall b.a \succ b \Rightarrow \aleph(b)) \Rightarrow
\aleph(a)\). L'absence de chaînes infiniment décroissantes fait que
...
est inclus dans l'arbre de~\(t\) et \(s \neq t\). Parfois, une forme
restreinte est suffisante. Par exemple, nous pouvons définir
\(\cons{x}{s} \succ s\)\index{cons@\fun{cons/2}}, pour tout
terme~\(x\) et toute pile~\(s \in S\). (À la fois \(x\)~et~\(s\) sont
des \emph{sous-termes immédiats}\index{terme!sous-terme immédiat} de
\(\cons{x}{s}\)\index{cons@\fun{cons/2}}.) Il n'existe pas de chaînes
infiniment décroissantes parce que \(\el\)~est l'unique élément
minimal de~\(S\): aucun~\(s\) ne satisfait \(\el \succ s\); donc le
cas de base est \(t=\el\) et \(\forall t.(\forall s.t \succ s
...
Prouvons l'associativité\index{pile!concaténation!associativité}
\index{induction!exemple} de \fun{cat/2}\index{cat@\fun{cat/2}}, que
nous exprimons symboliquement par \(\pred{CatAssoc}{s,t,u} \colon
\fun{cat}(s,\fun{cat}(t,u)) \equiv
\fun{cat}(\fun{cat}(s,t),u)\)\index{CatAssoc@\predName{CatAssoc}|(},
où \(s\), \(t\) et~\(u\) sont des valeurs de pile. Nous employons le
principe d'induction bien fondée à la structure de~\(s\), donc nous
devons établir
\begin{itemize}
...
\end{itemize}
Le cas de base est direct: \(\fun{cat}(\el,\fun{cat}(s,t))
\xrightarrow{\smash{\alpha}} \fun{cat}(s,t)
\xleftarrow{\smash{\alpha}}
\fun{cat}(\fun{cat}(\el,t),u)\)\index{cat@\fun{cat/2}}. Supposons
maintenant \(\pred{CatAssoc}{s,t,u}\), appelé l'\emph{hypothèse
  d'induction}\index{induction!hypothèse d'$\sim$}, et prouvons
...
Écrire \(\fun{cat}(\cons{x}{s},\fun{cat}(t,u))
\xrightarrow{\smash{\beta}} \cons{x}{\fun{cat}(s,\fun{cat}(t,u))}\)
est incorrect parce que la sémantique de notre langage fonctionnel
exige que les arguments soient évalués avant l'appel. Néanmoins,
si~\(s\), \(t\) et~\(u\) ne sont pas des valeurs, il est suffisant
qu'elles en aient une pour que \(\pred{CatAssoc}{s,t,u}\) soit vrai
aussi; par exemple, si \(f(s)\) a la valeur~\(v\), alors
\(\pred{CatAssoc}{f(s),t,u}\) est vrai car
\(\fun{cat}(f(s),\fun{cat}(t,u)) \twoheadrightarrow
\fun{cat}(v,\fun{cat}(t,u)) \equiv \fun{cat}(\fun{cat}(v,t),u)
\twoheadleftarrow \fun{cat}(\fun{cat}(f(s),t),u)\). Par conséquent, la
terminaison est une propriété importante que nous devons établir.
...
\(\fun{ack}(4,3) \twoheadrightarrow 2^{2^{65536}} - 3\). Sa
...
réécrit en appels plus petits. Avec la règle~\(\iota\), nous avons
\(\fun{ack}(m+1,0) \succ \fun{ack}(m,1)\). Avec la règle~\(\kappa\),
nous avons \(\fun{ack}(m+1,n+1) \succ \fun{ack}(m+1,n)\) et
\(\fun{ack}(m+1,n+1) \succ \fun{ack}(m,p)\), pour toute valeur~\(p\),
en particulier quand \(\fun{ack}(m+1,n) \twoheadrightarrow
...
Le patron conceptuel en \Java qui modélise une pile\index{pile!$\sim$
  en \Java} s'appuie sur les méthodes polymorphes, les classes
récursives et les génériques. Une classe abstraite et générique
\texttt{Stack} capture l'essence d'une pile comme suit:
\begin{alltt}
// Stack.java
...
\texttt{Stack}, capturant la relation «Une pile vide est une pile.» La
classe \texttt{EStack} ne contient aucune donnée.
\begin{alltt}
// EStack.java
...
Pour illustrer les piles en \Java,\index{pile!concaténation!$\sim$ en
  \Java|(} rappelons-nous de la définition~\eqref{def_cat} à la
page~\pageref{def_cat}\index{cat@\fun{cat/2}|(} pour concaténer deux
piles:
\begin{equation*}
\fun{cat}(        \el,t) \xrightarrow{\smash{\alpha}} t;\qquad
\fun{cat}(\cons{x}{s},t) \xrightarrow{\smash{\beta}}
...
méthode de \texttt{EStack} qui retourne son argument:
\begin{alltt}
