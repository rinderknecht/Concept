...
\emph{nœuds}\index{arbre!nœud} et les segments qui connectent
deux nœuds sont nommés \emph{arcs}\index{arbre!arc}. Le nœud
au sommet (contenant un diamètre) est la
\emph{racine}\index{arbre!nœud!racine} et les nœuds situés au
plus bas (\(\bullet\)) sont les
\emph{feuilles}\index{arbre!nœud!feuille}. Tous les nœuds sauf
les feuilles sont reliés vers le bas à d'autre nœuds, appelés les
\emph{enfants}\index{arbre!nœud!enfant}. Vers le haut, chaque
nœud sauf la racine est connecté à un autre nœud, ou
\emph{parent}\index{arbre!nœud!parent}. Selon le contexte, un
...
% [0pt] of margin overhang
\centering
\includegraphics[bb=68 652 123 715]{tree}% normal is [.. .. .. 715]
...
un nœud avec un point (\texttt{.}) dans l'arbre, auquel cas les
...
deux à deux}\index{tri!clé!unicité}, et l'on prend la moyenne des coûts
de toutes leurs \emph{permutations}\index{permutation} (au nombre
...
[3,4])\)\index{cat@\fun{cat/2}|(}. On gagne en clarté en créant des
\emph{classes d'équivalence} de termes qui sont joignables. La
relation \((\equiv)\) est définie ainsi:
\index{équivalence@$a \equiv b$|see{équivalence d'expressions}}
\index{équivalence!$\sim$ d'expressions}
\begin{center}
