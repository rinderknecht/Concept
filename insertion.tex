...
\label{fig:isrt_312}
\index{tri par insertion!insertion simple!exemple}
\end{figure}
...
\label{eq:cost_isrt}
\end{equation}
Un coup d'œil à la définition de~\fun{ins/2} révèle que
...
\index{tri par insertion!insertion simple!coût minimal}
...
\index{tri par insertion!insertion simple!coût maximal}
...
l'équation~\eqref{eq:cost_isrt} \vpageref{eq:cost_isrt}, nous voyons
...
\vpageref{inv_isrt} et~\eqref{eq:rev0} \vpageref{eq:rev0}, et aussi
\(\C{\fun{cat}}{n} =
...
Le coût moyen satisfait l'équation~\eqref{eq:cost_isrt} parce que
...
\vpageref{eq:mean_isrt}, le coût moyen du tri de \(n\)~clés par
...
\begin{enumerate}
  ...
  \item Traditionnellement, les manuels sur l'analyse des algorithmes
    ...
    \(\OM{\fun{isrt}}{n}\)\index{isrt@$\OM{\fun{isrt}}{n}$} le
...
\vpageref{par:cut_sound}, nous devons exprimer ici les propriétés
...
la~\vpageref{def:stack}, d'une manière qui engendre un ordre bien
...
section~\ref{sec:cutting}, \vpageref{sec:cutting}, en utilisant la
...
\begin{figure}
\centering
\subfloat[Arbre étendu\label{fig:perm_proof}]{
  \includegraphics[bb=135 672 369 723]{proof}}
\qquad
\subfloat[Arbre taillé\label{fig:312eq231}]{
...
\fun{cut/2}\index{cut@\fun{cut/2}}, \vpageref{par:cut_sound}, est que
...
\begin{itemize}
  ...
  \item Les axiomes \TirName{Pnil} et \TirName{Swap} prouvent
...
\begin{itemize}
  ...
  \item Si \(\Delta\) se termine avec \TirName{Trans}, alors
    l'hypothèse d'induction appliquée aux prémisses impliquent \(u
    \approx s\) et \(t \approx u\), qui peuvent être les prémisses de
    la règle \TirName{Trans} et conduisent à \(t \approx
    s\).\hfill\(\Box\)
...
de preuve}, comme\index{Coq@\textsf{Coq}}
...
\fig~\vref{fig:ver}.)\index{tri par insertion!insertion
...
\vpageref{sec:queueing}, nous avions utilisé deux piles pour simuler
une file.)

Appelons \(\fun{rcat}(t,u)\)\index{rcat@\fun{rcat/2}} la \emph{pile
simulée}\index{pile!$\sim$ simulée}; la pile~\(t\) est un
...
\end{array}
\end{equation*}
Étant donnée une clé~\(x\), elle est alors aisément insérée soit
dans~\(t\) (en faisant attention que l'ordre est inversé), ou bien
...
(préfixe retourné) et \(u\)~est la pile de droite (le suffixe).

La fonction \fun{i2w/1}\index{i2w@\fun{i2w/1}} est définie à la
...
\mypar{Coûts extrêmes}
\index{tri par insertion!insertion bidirectionnelle!coût minimal|(}
...
\index{tri par insertion!insertion bidirectionnelle!coût maximal|(}
...
\noindent De peu, le premier cas produit donc le coût maximal:
\begin{equation*}
...
illustrées par l'\emph{arbre d'évaluation}\index{arbre!$\sim$
  d'évaluation} à la \fig~\ref{fig:2way_unbal},
\begin{figure}[!b]
\centering
...
nœuds externes\index{arbre!nœud!$\sim$ externe} (voir
page~\pageref{def:external_node}). Par exemple, \([a,b,c]\) et
\([c,b,a]\) sont des nœuds externes.

Le coût total est la \emph{longueur externe}\index{arbre
  binaire!longueur externe}\label{external_path_length} de l'arbre,
...
de toutes les traces possibles:\index{langage
...
\vpageref{eq:sum_of_squares}). Le coût du retournement final est aussi
...
\begin{enumerate}
  ...
  \item Dans la conception de~\fun{i2w/3}\index{i2w@\fun{i2w/3}}, nous
    ...
    \begin{equation*}
      ...
      \end{array}}
    \end{equation*}
    \caption{Variation \fun{i2w\(_1\)/1} sur \fun{i2w/1} (voir (\(\leadsto\)))}
    \label{fig:i2w1}
    \end{figure}
  ...
  \item À la règle~\(\upsilon\) de~\fun{i2w/3}\index{i2w@\fun{i2w/3}},
    ...
    \begin{equation*}
      ...
      \end{array}}
    \end{equation*}
    \caption{Variation \fun{i2w\(_2\)/1} sur \fun{i2w/1} (voir (\(\leadsto\)))}
    \label{fig:i2w2}
    \end{figure}
...
Continuons en cherchant le coût minimal de \fun{i2wb/1}. Supposons que
...
l'équation~\eqref{eq:Mpq} \vpageref{eq:Mpq}. Par conséquent,
\begin{equation*}
