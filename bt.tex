...
La \fig~\vref{fig_bt_shape} montre un arbre binaire en exemple.
\begin{figure}
\centering
\includegraphics{bt_shape}
\caption{Un arbre binaire\label{fig_bt_shape}}
\end{figure}
...
\caption{Un arbre feuillu\label{fig_leaf_tree}}
\end{figure}
%\end{wrapfigure}
...
celui montré à la \fig~\vref{fig_bt_ex1}:
\begin{figure}
\centering
\subfloat[Arbre binaire étendu\label{fig_bt_ex1}]{
\includegraphics{bt_ex1}}
\qquad
\subfloat[Arbre binaire taillé\label{fig_bt_ex2}]{
\includegraphics[bb=67 645 169 721]{bt_ex2}}
\caption{Deux représentations d'un arbre binaire\label{fig_bt_ex}}
\end{figure}
...
externes peuvent être omis, comme à la \fig~\vref{fig_bt_ex2}. De
...
auquel cas l'arbre est dit \emph{feuillu}\index{arbre binaire!$\sim$
  feuillu}, comme à la \fig~\vref{fig_leaf_tree}. Dans une autre
...
\fig~\vref{fig_cmp_tree}, où les n{\oe}uds externes contiennent des
...
moyenne, \vpageref{par_minimax}, un \emph{chemin
...
à la \fig~\vref{fig_bt_shape} est~\(5\) et il y a deux chemins
...
l'arbre binaire de la \fig~\vref{fig_bt_ex1} correspond à
...
préfixe de l'arbre à la \fig~\vref{fig_bt_ex1} sont
...
en~\eqref{def_cat} \vpageref{def_cat}, pour ordonner les valeurs des
...
chapitre~\vref{chap_merge_sort}, qui souvent symbolise le concept
lui-même.
...
montrée à la \fig~\vref{fig_max_pre0}.
\begin{figure}
\centering
\subfloat[Maximum\label{fig_max_pre0}]{
  \includegraphics{max_pre0}}
\qquad
\subfloat[Minimum\label{fig_min_pre0}]{
  \includegraphics{min_pre0}}
\caption{Arbres extrêmes pour \(\C{\fun{pre}_0}{n}\)
\label{fig_tree_stack}}
\end{figure}
...
montrée à la \fig~\vref{fig_min_pre0}. Notez que les deux arbres
...
à la \fig~\vref{fig_flat}, \emph{une seule fois à la fin}. La
\fig~\vref{fig_pre1} montre qu'une nouvelle version du parcours
...
\end{array}}
\end{equation*}
\caption{Parcours préfixe avec \fun{flat/1}\label{fig_pre1}}
\end{figure}
...
\end{array}}
\end{equation*}
\caption{Parcours préfixe efficace avec une forêt\label{fig_pre3}}
\end{figure}
...
premier arbre) dans l'ordre attendu. Nous donnons un exemple à la \fig~\vref{fig_pre3_abcde},
\begin{figure}
\centering
...
comme montré à la \fig~\ref{fig_pre5}.
\begin{figure}[b]
\begin{equation*}
\boxed{
\begin{array}{r@{\;}c@{\;}l}
...
\end{array}}
\end{equation*}
\caption{Longue définition d'un parcours préfixe\label{fig_pre5}}
\end{figure}
...
\fig~\vref{fig_tree_stack}. Dans le pire des cas, tous les \(2n+1\)
...
l'arbre de la \fig~\vref{fig_max_pre0}. Nous avons rencontré ce genre
de réécriture partielle dans la
formule~\eqref{eq_rev0}\index{rev0@\fun{rev\(_0\)/1}}
\vpageref{eq_rev0} et~\eqref{inv_isrt}
...
\(t=\fun{int}(x,t_1,t_2)\) à la \fig~\vref{fig_pre_tree}.
\begin{figure}
\centering
\subfloat[Préfixe avec \fun{pre/1}\label{fig_pre_tree}]{
  \includegraphics[bb=71 672 166 721]{pre_tree}}
\subfloat[Infixe avec \fun{in/1}\label{fig_in_tree}]{
  \includegraphics[bb=71 672 183 721]{in_tree}}
\subfloat[Postfixe avec \fun{post/1}\label{fig_post_tree}]{
  \includegraphics[bb=71 672 183 721]{post_tree}}
\caption{Parcours classiques efficaces\label{fig_classic_walks}}
\end{figure}
...
rédiger le programme fonctionnel à la \fig~\vref{fig_pre}.
\begin{figure}[b]
\begin{equation*}
\boxed{
\begin{array}{r@{\;}c@{\;}l}
...
\end{array}}
\end{equation*}
\caption{Parcours préfixe efficace (coût et mémoire)\label{fig_pre}}
\end{figure}
...
\fig~\vref{fig_pre3} empile \(t_1\) et~\(t_2\), donc alloue deux
...
La \fig~\vref{fig_preorder}
\begin{figure}
\centering
\subfloat[Préfixe\label{fig_preorder}]{\includegraphics{preorder}}
\;
\subfloat[Postfixe\label{fig_postorder}]{\includegraphics{postorder}}
\;
\subfloat[Infixe\label{fig_inorder}]{\includegraphics{inorder}}
\;
\subfloat[Par niveau\label{fig_lorder}]{\includegraphics{lorder}}
...
\caption{Rangs préfixes en exposant\label{fig_bt_ex3}}
\end{figure}
...
l'{\oe}uvre sur les n{\oe}uds internes à la \fig~\vref{fig_prenum1}.
\begin{figure}[t]
\centering
\subfloat[Rangs seuls\label{fig_prenum1}]{%
\includegraphics[bb=71 668 170 721]{prenum1}}
\qquad
\subfloat[Rangs et arbre\label{fig_prenum0}]{%
\includegraphics[bb=71 668 217 721]{prenum0}}
\caption{Numérotation préfixe en deux temps\label{fig_prenum}}
\end{figure}
...
\fig~\vref{fig_prenum0}. Conceptuellement, il s'agit-là de la
...
\emph{number in preorder}) à la \fig~\vref{fig_npre}
\begin{figure}[t]
...
\caption{Numérotation préfixe\label{fig_npre}}
\end{figure}
...
\vpageref{par_infsys}, peuvent être éliminés en introduisant des
...
\end{equation*}
\caption{Version de \fun{npre/1} sans système d'inférence\label{fig_npre_bis}}
\end{figure} 
\index{arbre binaire!préfixe|)}
...
page~\pageref{par_ackermann} est pertinente ici aussi. Nous
...
de~\(s\) (page~\pageref{par_well-founded}). D'après la définition à la
\fig~\vref{fig_pre}, nous voyons que la règle~\(\theta\) conserve la
...
l'équation~\eqref{eq_pre0} \vpageref{eq_pre0}, et celle de
...
%\fig~\vref{fig_CatPre},\index{CatPre@\predName{CatPre}}
%\begin{figure}[t]
\begin{equation*}
%\boxed{%
\begin{array}{r@{\;}c@{\;}l}
...
%We can resume conclusively in \fig~\vref{fig_pre0_pre}.\hfill\(\Box\)
%\begin{figure}
\begin{equation*}
%\boxed{%
\begin{array}{r@{\;}c@{\;}l@{\qquad}r}
...
%  \fun{pre}(t)\) \label{fig_pre0_pre}}
%\end{figure}

\paragraph{Aplatissement revu}
\index{pile!aplatissement|(}

À la section~\vref{sec_flattening}, nous avons défini deux fonctions
pour aplatir des piles\index{pile!aplatissement} (voir
\fig~\ref{fig_flat0} à la page~\pageref{fig_flat0} et la
\fig~\ref{fig_flat} à la page~\pageref{fig_flat}). Avec le parcours
...
parcourir en préfixe un arbre feuillu\index{arbre binaire!$\sim$
  feuillu} (voir \fig~\vref{fig_leaf_tree} pour un exemple), en
...
\caption{Piles imbriquées vues comme un arbre feuillu\label{fig_stacks}}
\end{figure}
...
\fig~\vref{fig_pre}, de telle sorte qu'elle traite des arbres binaires
feuillus. La nouvelle fonction, \fun{lpre/1}\index{lpre@\fun{lpre/1}},
est montrée à la \fig~\vref{fig_lpre}.
\begin{figure}
\begin{equation*}
\boxed{
\begin{array}{r@{\;}c@{\;}l}
...
\end{array}}
\end{equation*}
\caption{Parcours préfixe sur des arbres binaires feuillus\label{fig_lpre}}
\end{figure}
...
\end{array}}
\end{equation*}
\caption{Aplatir comme \fun{lpre/1}\label{fig_flat2}}
\end{figure}
...
l'équation~\eqref{eq_pre} \vpageref{eq_pre}, où \(n\)~est le nombre de
...
infixe de l'arbre à la \fig~\vref{fig_bt_ex1} est
...
\fun{pre/1}\index{pre@\fun{pre/1}} à la \fig~\vref{fig_pre}, nous
comprenons que nous devrions structurer notre programme de telle sorte
qu'il suive la stratégie montrée à la \fig~\vref{fig_in_tree}, où la
seule différence avec la \fig~\ref{fig_pre_tree} est le moment où la
...
\caption{Numérotation infixe\label{fig_nin}}
\end{figure}
...
\fig~\vref{fig_flat} peut suggérer une nouvelle approche du parcours
infixe. En composant des rotations à droite\index{arbre
  binaire!rotation|(}, vues aux \figs~\vrefrange{fig_lift1}{fig_lift2}
...
\label{def_Rot}
\end{equation*}
À la \fig~\vref{fig_rot}, nous montrons comment un arbre binaire
...
\centering
\includegraphics{rot}
\caption{Rotations à droite descendantes\label{fig_rot}}
\end{figure}
...
droite est montrée à la \fig~\vref{fig_in1}.
\begin{figure}
\begin{equation*}
\boxed{
\begin{array}{r@{\;}c@{\;}l}
...
\end{array}}
\end{equation*}
\caption{Parcours infixe par rotations à droite\label{fig_in1}}
\end{figure}
...
(par exemple, dans l'arbre initial à la \fig~\vref{fig_rot}, la
...
définition~\eqref{def_rev} \vpageref{def_rev}). Cette propriété est
...
exemple, le parcours postfixe de l'arbre à la \fig~\vref{fig_bt_ex1}
...
n{\oe}uds internes à la \fig~\vref{fig_post_tree}. La différence avec
...
définition de la fonction est donnée à la \fig~\vref{fig_post}.
\begin{figure}
%\abovedisplayskip=0pt
%\belowdisplayskip=0pt
\begin{equation*}
...
\end{array}}
\end{equation*}
\caption{Parcours postfixe\label{fig_post}}
\end{figure}
...
\fig~\vref{fig_postorder}, donc le parcours postfixe de cet arbre est
\([0,1,2,3,4,5,6]\). Remarquons que les rangs augmentent le long de
chemins ascendants. La
\fig~\vref{fig_npost}\index{npost@\fun{npost/1}}\index{npost@\fun{npost/2}}
\begin{figure}
%\abovedisplayskip=0pt
%\belowdisplayskip=0pt
\centering
...
\caption{Numérotation postfixe\label{fig_npost}}
\end{figure}
...
définition~\eqref{eq_BS_order} \vpageref{eq_BS_order}. L'élément
...
\fun{int}(x,t_1,t_2)\). %À la \fig~\vref{fig_premir},
...
%}
\end{equation*}
%\caption{Preuve de \(\fun{pre}(\fun{mir}(t),s) \equiv %\fun{rcat}(\fun{post}(t,\el),s)\)\label{fig_premir}}
%\end{figure}
où
\begin{itemize}
...
niveaux de l'arbre à la \fig~\vref{fig_bt_ex1} est
...
À la \fig~\vref{fig_lorder} est montrée une \emph{numérotation par
  niveaux} de l'arbre à la \fig~\vref{fig_bt_ex3}, souvent appelée
...
exemple, \fun{size/1}\index{size@\fun{size/1}} dans~\eqref{eq_size}
\vpageref{eq_size}.
...
partiellement définie à la \fig~\vref{fig_lo0}.
\begin{figure}[t]
\centering
\includegraphics[bb=108 682 395 721]{lo0}
\caption{Parcours par niveaux avec \fun{bf\(_0\)/1}\label{fig_lo0}}
\end{figure}
...
(anglais, \emph{deforest}), définie à la \fig~\vref{fig_def},
\begin{figure}
\abovedisplayskip=0pt
\belowdisplayskip=0pt
\centering
\includegraphics[bb=71 660 324 721]{def}
\caption{Coupe\label{fig_def}}
\end{figure}
...
la \fig~\vref{fig_def}, la règle d'inférence augmente le niveau
...
étendu~\(2\) de l'arbre à la \fig~\vref{fig_bt_ex3} est \([{}^{2}{3},
...
\fig~\ref{fig_bt_ex} \vpageref{fig_bt_ex}. Remarquons qu'il y a
...
pleins (voir \fig~\vref{fig_comp_tree}). Dans ce cas, \(l_i=2^i\),
...
comme ceux montrés à la \fig~\vref{fig_tree_stack} et à la
\fig~\vref{fig_zigzag}. Ici, \(h=n\) et le coût est
...
\fig~\vref{fig_pre3}. La différence est qu'au lieu d'utiliser une pile
...
nous avons présentée à la section~\ref{sec_queueing}. Considérons à la
\fig~\vref{fig_lo_abcde} l'algorithme à l'{\oe}uvre sur le même
exemple qu'à la \fig~\vref{fig_pre3_abcde}.
\begin{figure}[b]
\centering
\includegraphics[bb=70 655 385 723]{lo_abcde_0}
\bigskip
\includegraphics[bb=70 690 388 723]{lo_abcde_1}
\caption{Parcours des niveaux avec une file\label{fig_lo_abcde}}
\end{figure}
...
\fig~\vref{fig_pre3}, nous écrirons
...
un parcours des niveaux à la \fig~\ref{fig_bf1}.
\begin{figure}
\begin{equation*}
\boxed{
\begin{array}{r@{\;}c@{\;}l}
...
\end{array}}
\end{equation*}
\caption{Parcours des niveaux avec une file abstraite\label{fig_bf1}}
\end{figure}
...
À la section~\ref{sec_queueing}, nous avons vu comment mettre en
...
dans~\eqref{def_enq}, \vpageref{def_enq}, et le défilage avec
\fun{deq/1}\index{deq@\fun{deq/1}} dans~\eqref{def_deq}. Ceci nous
permet de raffiner la définition
de~\fun{bf\(_2\)/1}\index{bf2@\fun{bf\(_2\)/1}} en
\fun{bf\(_3\)/1}\index{bf3@\fun{bf\(_3\)/1}} à la \fig~\vref{fig_bf3}.
\begin{figure}
\abovedisplayskip=0pt
\belowdisplayskip=0pt
\centering
...
\end{gather*}}}
\caption{Raffinement de la \fig~\vref{fig_bf1}\label{fig_bf3}}
\end{figure}
...
est montré à la \fig~\vref{fig_bf}.
\begin{figure}
\begin{equation*}
\boxed{
\begin{array}{r@{\;}c@{\;}l}
...
\end{array}}
\end{equation*}
\caption{Raffinement de la \fig~\vref{fig_bf3}\label{fig_bf}}
\end{figure}
Souvenons-nous que \fun{rcat/2}\index{rcat@\fun{rcat/2}} (anglais,
\emph{reverse and catenate}) est définie à l'équation~\eqref{def_rev}
\vpageref{def_rev}. Remarquons que
...
est définie par~\eqref{eq_size} \vpageref{eq_size}. Ceci est une sorte
...
exemple d'automate fini déterministe\index{automate fini!$\sim$
  déterministe|(} (AFD) a été vu à la \fig~\vref{fig_abacabac}. Ici,
...
la \fig~\vref{fig_bf}, nous voyons que
\begin{itemize}
...
\end{itemize}
À la \fig~\vref{fig_lo_nfa},
\begin{figure}
\centering
\subfloat[Non-déterministe (AFN)\label{fig_lo_nfa}]{
\includegraphics[bb=71 580 209 721]{lo_nfa}}
\quad
\subfloat[Déterministe (AFD)\label{fig_lo_dfa}]{
...
alors la \fig~\vref{fig_lo_dfa} montre un AFD équivalent pour les
...
Comme nous l'avons mentionné plus tôt, la \fig~\vref{fig_lorder}
...
\fun{bf\(_1\)/2}\index{bf1@\fun{bf\(_1\)/2}} à la \fig~\vref{fig_bf1}
...
\emph{breadth-first numbering}) à la \fig~\vref{fig_bfn1},
\begin{figure}
\abovedisplayskip=0pt
\belowdisplayskip=0pt
\centering
...
\end{gather*}}}
\caption{Numérotation en largeur abstraite\label{fig_bfn1}}
\end{figure}
et comparons-les avec les définitions à la \fig~\vref{fig_bf1}. En
...
\caption{Exemple de numérotation en largeur\label{fig_bf2_ex}}
\end{figure}
...
cela est montré à la \fig~\vref{fig_bfn3},
\begin{figure}[!t]
\centering
\includegraphics[bb=71 599 403 721]{bfn3}
\caption{Raffinement de la \fig~\vref{fig_bfn1}\label{fig_bfn3}}
\end{figure}
ce qui pourrait être contrasté avec la \fig~\vref{fig_bf3}.
...
(voir \fig~\vref{fig_comp_tree}). Une façon de définir de tels arbres
...
hauteur d'un n{\oe}ud externe est~\(0\).) À la \fig~\vref{fig_per} est
...
\centering
\includegraphics{per}
\caption{Vérification de la perfection\label{fig_per}}
\end{figure}
...
\fig~\vref{fig_per0}, où \(\fun{per}_0(t_1)\) est évalué avant
...
\caption{Raffinement de la \fig~\ref{fig_per}\label{fig_per0}}
\end{figure}
...
à la \fig~\ref{fig_complete}. Récursivement, un arbre donné est
...
complets. Le programme se trouve à la \fig~\vref{fig_comp}. La
...
\caption{Verification de la complétude\label{fig_comp}}
\end{figure}
...
similaires. Voir la \fig~\vref{fig_bal} pour un exemple. Commençons
...
\end{equation*}
La modification est montrée à la \fig~\vref{fig_balance},
\begin{figure}
\centering
\includegraphics{balance}
\caption{Vérification de l'équilibre\label{fig_balance}}
\end{figure}
...
(anglais, \emph{extended postorder})\index{arbre binaire!parcours
  postfixe!codage} à la \fig~\vref{fig_epost},
\begin{figure}[b]
\begin{equation*}
\boxed{
\begin{array}{r@{\;}c@{\;}l}
...
\end{array}}
\end{equation*}
\caption{Codage postfixe\label{fig_epost}}
\end{figure}
est une simple modification de \fun{post/1}\index{post@\fun{post/1}} à
la \fig~\vref{fig_post}. Par exemple, l'arbre à la
\fig~\vref{fig_postorder} est codé par \([\Box, \Box, \Box, 0, 1,
...
\fig~\vref{fig_post2b}.\index{arbre binaire!postfixe!décodage}
\begin{figure}
\begin{equation*}
\boxed{
\begin{array}{r@{\;}c@{\;}l}
...
\end{array}}
\end{equation*}
\caption{Décodage postfixe\label{fig_post2b}}
\end{figure}
...
(anglais, \emph{extended preorder}) à la \fig~\vref{fig_epre}
\begin{figure}[b]
\begin{equation*}
\boxed{
\begin{array}{r@{\;}c@{\;}l}
...
\end{array}}
\end{equation*}
\caption{Codage préfixe\label{fig_epre}}
\end{figure}
est une simple modification de \fun{pre/1}\index{pre@\fun{pre/1}} à la
\fig~\vref{fig_pre}. Le coût est aussi simple à déterminer que pour un
...
\index{rev@\fun{rev/1}}
\end{equation}
À la section~\ref{sec_reversal}, nous avons prouvé
...
\end{array}}
\end{equation*}
\caption{Décodage préfixe\label{fig_pre2b}}
\end{figure}
...
suffixe. La définition est montrée à la \fig~\vref{fig_pre2b0}.
\begin{figure}
\abovedisplayskip=0pt
\belowdisplayskip=0pt
\centering
...
\end{gather*}}}
\caption{Un autre décodage préfixe\label{fig_pre2b0}}
\end{figure}
...
avons mentionné que l'arbre à la \fig~\vref{fig_postorder} a pour
...
par~\(1\), donc l'arbre de la \fig~\vref{fig_preorder} donne \([0, 1,
...
algorithme en-ligne (voir \vpageref{par_online_vs_offline} et
section~\ref{sec_online} \vpageref{sec_online}) nécessite de garder un
...
coût amorti plus faible (voir page~\pageref{par_amortised_cost}) ou
d'un coû moyen inférieur (voir l'insertion bidirectionnelle à la
section~\ref{sec_2-way} \vpageref{sec_2-way}).

\begin{wrapfigure}[7]{r}[0pt]{0pt}
\centering
\includegraphics[bb=60 663 120 721]{binzip}
\caption{\label{fig_binzip}}
\end{wrapfigure}
...
\fig~\ref{fig_binzip}, où le curseur est le n{\oe}ud~\fun{d}. La
...
\end{array}}
\end{equation*}
\caption{Déplacements dans un arbre binaire\label{fig_zip_move}}
\end{figure}
...
est montré à la \fig~\vref{fig_pre_tail},
\begin{figure}[b]
\begin{equation*}
\boxed{
\begin{array}{r@{\;}c@{\;}l}
...
\end{array}}
\end{equation*}
\caption{Parcours préfixe en forme terminale\label{fig_pre_tail}}
\end{figure}
...
donné à la \fig~\vref{fig_dyck_path}, sans prendre en compte les coûts
de chaque pas.

Si nous suivons la même convention qu'au
chapitre~\ref{chap_factoring}, nous dirions ici qu'un \emph{mot de
...
au chemin de Dyck à la \fig~\vref{fig_dyck_path} est
...
l'arbre à la \fig~\vref{fig_dyck2bt}
\begin{figure}[b]
\centering
\subfloat[Arbre binaire\label{fig_dyck2bt}]{%
\includegraphics[bb=71 655 142 721]{dyck2bt}}
\qquad\quad
\subfloat[Chemin de Dyck correspondant au codage préfixe $[0,1,\Box,2,\Box,\Box,3,\Box,\Box\char93$\label{fig_bt2dyck}]{%
...
\fig~\vref{fig_bt2dyck}\index{arbre binaire!préordre!codage} de la
façon suivante:
\begin{align*}
\delta(t)
...
\fun{epre/2}\index{epre@\fun{epre/2}} à la \fig~\vref{fig_epre} pour
...
\fun{dpre/1}\index{dpre@\fun{dpre/1}} (anglais, \emph{Dyck path as
  preorder}) à la \fig~\vref{fig_dpre}.
\begin{figure}
\begin{equation*}
\boxed{
\begin{array}{r@{\;}c@{\;}l}
...
\end{array}}
\end{equation*}
\caption{Codage préfixe d'un arbre en un chemin de Dyck\label{fig_dpre}}
\end{figure}
...
\figs~\vrefrange{fig_pre2b}{fig_pre2b0},\index{pre2b@\fun{pre2b/1}}
...
l'avons vu aux
\figs~\vrefrange{fig_epost}{fig_post2b}. \index{epost@\fun{epost/1}}\index{post2b@\fun{post2b/1}}
...
aller directement de l'arbre binaire au chemin de Dyck\index{chemin de
  Dyck}, comme à la
\fig~\vref{fig_dpost}.\index{dpost@\fun{dpost/1}}\index{dpost@\fun{dpost/2}}
\begin{figure}[t]
\begin{equation*}
\boxed{
\begin{array}{r@{\;}c@{\;}l}
...
\fun{dpre/2}\index{dpre@\fun{dpre/2}} à la \fig~\vref{fig_dpre}.) En
...
\fig~\vref{fig_post2b}: nous initialisons la pile auxiliaire avec
\([\fun{ext}()]\). La fonction est nommée
\fun{d2b/1}\index{d2b@\fun{d2b/1}} et définie à la
\fig~\vref{fig_d2b}. Le coût est \(\C{\fun{d2b}}{n} = 2n +
...
\end{array}}
\end{equation*}
\caption{Décodage postfixe d'un chemin de Dyck en un arbre\label{fig_d2b}}
\end{figure}
...
que de chemins de Dyck de longueur~\(2n\). Nous savons déjà, voir le chapitre~\ref{chap_Catalan}, et l'équatio~\eqref{eq_Cn}
\vpageref{eq_Cn}, qu'il y a \(C_n = \frac{1}{n+1}\binom{2n}{n}
...
n{\oe}ud externe, à la section~\vref{sec_opt_sort} à propos du tri
...
\fig~\vref{fig_pre3} est deux fois la taille moyenne de la file
nécessaire au parcours en largeur avec \fun{bf/1}\index{bf@\fun{bf/1}}
à la \fig~\vref{fig_bf}. Ceci n'est pas du tout évident, car les deux
