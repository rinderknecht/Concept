...
\label{fig:bt_shape}
\end{figure}
Les nœuds sont de deux sortes: internes\index{arbre
  binaire!nœud interne|(} (\(\circ\) et \(\bullet\)) ou
externes\index{arbre binaire!nœud externe|(} (\(\scriptstyle
\Box\)). La caractéristique d'un arbre binaire est que les nœuds
internes sont reliés vers le bas à deux autres nœuds, appelé
\emph{enfants}\index{arbre!nœud!enfant}, alors que les nœuds
externes n'ont pas de tels liens. La
racine\index{arbre!nœud!racine} est le nœud interne le plus
haut, représenté par un cercle et un diamètre. Les
\emph{feuilles}\index{arbre binaire!feuille} sont les nœuds
...
nœud externe\index{arbre!nœud!$\sim$ externe} et la
...
nœuds externes et \fun{int/3}\index{int@\fun{int/3}} pour les
...
la convention \(\fun{int}(x,t_1,t_2)\), parfois appelée \emph{notation
  ...
  binaire!nœud interne|)}\index{arbre binaire!nœud externe|)}
...
d'un nœud avant ses frères\index{arbre!nœud!frère} (deux
...
coût maximal de \fun{cat/2}\index{cat@\fun{cat/2}} à chaque nœud,
...
\emph{peignes}.\index{arbre binaire!$\sim$ dégénéré} Par ailleurs, le
coût maximal de \fun{pre\(_0\)/1}\index{pre0@\fun{pre\(_0\)/1}} est
...
dans lequel on choisit des nœuds internes facilement (la racine du
...
\end{figure}
où la forêt est l'argument de \fun{pre\(_4\)/1} et les nœuds
...
Au total, le nombre de nœuds d'empilage\index{nœud d'empilage}
...
Examinons le nœud interne \(t=\fun{int}(x,t_1,t_2)\) à la
...
\;
\subfloat[Nivelé\label{fig:lorder}]{\includegraphics{lorder}}
...
conséquence, \fun{pre/1}\index{pre@\fun{pre/1}} termine pour toutes
les données.\index{arbre
...
\begin{equation*}
  ...
  \fun{pre}(\fun{ext}()).
\end{equation*}
Voir la définition de \fun{pre\(_0\)/1} à
...
pour aplatir des piles\index{pile!aplatissement}
...
parcourir en préfixe un arbre feuillu\index{arbre binaire!$\sim$
  feuillu} (voir \fig~\vref{fig:leaf_tree}), en laissant de côté les
...
nœuds internes sont des \emph{fourches}\index{arbre
...
La conception de
\fun{flat/1}\index{pile!applatissement}\index{flat@\fun{flat/1}} à la
...
devient un arbre dégénéré\index{arbre binaire!$\sim$ dégénéré} (ou
...
rallonge d'un nœud exactement la \emph{branche la plus à
  droite}\index{arbre!branche}, c'est-à-dire la suite de nœuds
...
Un \emph{parcours des niveaux}\index{arbre binaire!parcours des
  niveaux} consiste en une pile contenant les nœuds de tous les
...
d'un niveau ne sont pas frères\index{arbre!nœud!frère}, sauf au
...
travailler avec les \emph{niveaux étendus}\index{arbre binaire!niveau
  étendu}. Un niveau étendu est un niveau où les nœuds externes
...
\fun{bf/1}\index{bf@\fun{bf/1}} peut être considérée comme le plus
...
+ 3\lg(n+1) + 3 \sim 4n\).\index{bf@$\B{\fun{bf}}{n,h}$} Le coût
...
arbre dégénéré (un peigne). Par conséquent, \fun{bf/1} est préférable
dans tous les cas.
...
\fun{bf\(_1\)/2}\index{bf1@\fun{bf\(_1\)/2}}, les nœuds externes
...
de tous les nœuds ont la même hauteur\index{arbre!hauteur}. (La
...
% [0pt] of margin overhang
\centering
\includegraphics[bb=71 653 147 719]{complete}
\caption{}
\label{fig:complete}
\end{wrapfigure}
Un arbre binaire est \emph{complet}\index{arbre binaire!$\sim$
  complet} si les enfants de chaque nœud interne sont deux nœuds
...
\begin{wrapfigure}[7]{r}[0pt]{0pt}
\centering
\includegraphics[bb=71 653 153 719]{bal}
\caption{}
\label{fig:bal}
\end{wrapfigure}
...
\end{equation*}
La modification est montrée à la \fig~\vref{fig:balance}, où la règle
d'inférence de \fun{bal\(_0\)/1}\index{bal0@\fun{bal\(_0\)/1}} est
requise pour vérifier la condition \(|h_1 - h_2| \leqslant
1\). Remarquons qu'un arbre parfait est équilibré (\(h_1 = h_2\)).
...
\(\pred{EqRev}{s}\)\index{EqRev@\predName{EqRev}}, soit l'involution
de
...
\centering
\includegraphics[bb=62 663 128 712]{binzip}
\caption{}
\label{fig:binzip}
\end{wrapfigure}
Appelons le nœud courant le \emph{curseur}\index{arbre
...
\fig~\ref{fig:binzip}, où le curseur est le nœud~\fun{d}. La
...
\fun{ext}()))\) signifie que le nœud~\fun{b} possède un enfant à
...
nœud~\fun{a} a un enfant à droite~\fun{g} qui n'a pas été
visité. La pile \([p_1,p_2]\) est le chemin du nœud~\fun{d} vers
...
Ces mots sont appelés \emph{premiers}, parce que tout mot de Dyck peut
...
\emph{décomposition en arches}\index{chemin de
...
\fun{ext}()),
\end{equation*}
% \(t=\fun{int}(0, \fun{int}(1, \fun{ext}(), \fun{int}(2,
...
\vpageref{eq:Cn}, qu'il y a \(C_n\) de ces chemins:
\begin{equation*}
