...
les nœuds d'empilage\index{nœud d'empilage}.
...
nœuds d'empilage\index{nœud d'empilage} créés par la
...
d'empilage\index{nœud d'empilage} pour chaque clé, ce qui nous
...
\end{figure}
En prenant comme origine le nœud \fun{sum}, l'arbre de syntaxe
...
d'empilage\index{nœud d'empilage} à devenir une addition. Par
...
\caption{Définition de \fun{sum/1} avec partage maximal}
\label{fig:sum_dag}
\end{figure}
...
remplacer l'arc vers le nœud \fun{sum} précédent par un arc vers
...
nœuds \fun{sum} et~(\(+\)) deviennent inutiles, pas à pas,
...
dès que possible et trois nœuds \fun{sum} sont réclamés par le
...
le nœud \fun{sum}), pour faire court. \emph{Un nœud est utile
...
pouvons prédire exactement quand les nœuds \fun{sum} peuvent être
réclamés: après chaque pas en arrière (de la droite vers la gauche à
la \fig~\ref{fig:sum375_pop}), le nœud \fun{sum} le plus à droite
...
\begin{enumerate}
  ...
  \item Tant que la pile de contrôle contient au moins deux objets,
    ...
    \item si la racine de l'arbre au sommet est un nœud \fun{sum},
      alors nous le dépilons et empilons à sa place la valeur;

    \item sinon, le nœud \fun{sum} dans l'arbre possède un arc
      entrant:
      \begin{enumerate}
...
Dans le but de déterminer le coût minimal de la version finale de
...
raisonner que ce coût minimal de \erlcode{isrt\_tf/1} est aussi un
...
Soit \(\W{\fun{isrt\_tf}}{n}\) le coût maximal de
...
Pour obtenir un résultat plus fort, nous avons besoin d'une analyse du
coût moyen, de façon à départager \erlcode{isrt\_tf/1} et
\erlcode{isrt/1}. En effet, il se pourrait que, pour une entrée de
longueur~\(n\), la plupart des configurations de l'entrée mènent à un
coût de \erlcode{isrt\_tf/1} qui est en fait inférieur à celui de
\erlcode{isrt/1}.

Notons \(\M{\smash[t]{\fun{isrt\_tf}}}{n}\) la nombre moyen de
réécritures nécessaires pour évaluer \erlcode{isrt\_tf}{(\(S\))}, où
...
Pour l'illustrer, améliorons donc le programme que nous avons obtenu
précédemment: \index{pile!aplatissement|(}
\begin{alltt}
flat0\_tf(T)          \(\smashedrightarrow{\alpha}\) flat0(T,[]).
flat0(         [],A) \(\smashedrightarrow{\gamma}\) appk([],A);
flat0(     [[]|T],A) \(\smashedrightarrow{\delta}\) flat0(T,A);
...
\end{equation*}
Ajoutons~\(1\) aux deux côtés et réordonnons les termes:
\begin{equation*}
\C{\fun{fib}}{n} + 1
...
à la racine. Elle est faite des nœuds successifs \erlcode{fib(1)},
...
\caption{Arbre des appels de \erlcode{fib(5)} avec partage maximal}
...
elle-même (un nœud). Dans nos deux exemples, \erlcode{F}~est en
...
doublons} dans un arbre binaire de recherche\index{arbre binaire de
  recherche} à la \fig~\vref{fig:insl0}:
\begin{alltt}
...
si~\erlcode{Y} est trouvée, l'entrée complète est partagée et
...
\erlcode{pal/1} et \erlcode{pal0/1} sont les mêmes. Le coût minimal
