...
La fonction à la \fig~\vref{fig_pre2b0} devient, quant à elle:
\begin{verbatim}
...
\figs~\vrefrange{fig_fail}{fig_mp_def}. En incluant ici la solution à
...
La définition de~\fun{rev\(_0\)/1} à la section~\ref{sec_reversal}:
\begin{equation*}
...
l'équation~\eqref{eq_fact_tf} \vpageref{eq_fact_tf}, l'idée est
...
À la section~\ref{sec_persistence}, nous avons supposé que le partage
...
\fig~\vref{fig_flat0} où la fonction \fun{flat\(_0\)/1} est
...
\vpageref{eq_push_pop_persistence} \emph{doivent} être traduites comme
suit:
\begin{verbatim}
push(X,H=[S|_]) -> [[X|S]|H].
pop(T=[[X|S]|_]) -> {X,[S|T]}.
...
de \fun{sum/1} à la \fig~\vref{fig_sum_dag}.
\begin{figure}
\centering
...
La phase de dépilage est montrée en action à la \fig~\vref{fig_sum375_pop},
\begin{figure}[b]
\centering
...
la \fig~\ref{fig_sum375_pop}), le n{\oe}ud \fun{sum} le plus à droite
...
Considérons comment le calcul à la \fig~\vref{fig_sum375_pop} peut
...
\begin{figure}[!t]
\centering
\subfloat[Fin de la phase 1/2\label{fig_cat1234_push}]{
\includegraphics[bb=71 615 287 734]{cat1234_push}
%\caption{Evaluation of \(\fun{cat}([1,2],[3,4])\) (end of phase 1/2)
%\label{fig_cat1234_push}}
}
\vskip32pt
%\end{figure}
%\begin{figure}
%\centering
\subfloat[Phase 2/2\label{fig_cat1234_pop}]{
...
Notre définition de \fun{flat/1} à la \fig~\vref{fig_flat}, est
...
est-il de \erlcode{flat0/1} à la \fig~\vref{fig_flat0}? Nous avons:
...
\erlcode{rcat/2} (section~\vref{sec_reversal}):
...
Transformons l'insertion simple (section~\ref{sec_straight_ins}
\vpageref{sec_straight_ins}) et analysons le coût de la définition
...
\begin{figure}
\centering
\subfloat[Avec une pile de \(n\)-uplets\label{fig_acc_stack}]{
  \includegraphics[bb=72 642 220 721]{acc_stack}
}
\subfloat[Avec des \(n\)-uplets imbriqués\label{fig_acc_tup}]{
...
\erlcode{slst0/2}, définie par l'équation~\eqref{eq_slst0}
\vpageref{eq_slst0}:
\begin{verbatim}
...
(section~\ref{sec_straight_ins} \vpageref{sec_straight_ins}) qui
...
\begin{verbatim}
 rm_keys(isrtf(mk_keys(fun len0/1,
               ...
               fun({K1,_},{K2,_}) -> K1 < K2 end))
\end{verbatim}
où \erlcode{len0/1} est définie à la \fig~\vref{fig_len0}.
\begin{figure}[b]
\begin{equation*}
\boxed{
\begin{array}{@{}r@{\;}l@{\;}l@{}}
...
\fig~\vref{fig_sum0} en la personne de \erlcode{sum0/1}. La première
...
est \fun{len0/1} à la \fig~\vref{fig_len0}. Dans ce cas,
...
l'inégalité~\eqref{ineq_cat_assoc} \vpageref{ineq_cat_assoc} le
démontre.
...
la section~\ref{sec_skipping}, \vpageref{sec_skipping}, est, en
\Erlang,
\begin{alltt}
...
Un effet secondaire positif de l'emploi de mappages et compositions
...
l'équation~\eqref{eq_foldlr} \vpageref{eq_foldlr}. La dérivation
...
\paragraph{Codage fonctionnel des mappages}
...
(\erlcode{find/2}) est maintenant une seule fonction, représentant le
...
\fig~\vref{fig_len0} où \erlcode{len0/2} est la fonction
...
\fig~\vref{fig_tms}: dupliquer la référence à~\erlcode{S} et nous
