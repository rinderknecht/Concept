...
exemple\index{pile!filtrage!exemple} d'une recherche qui trouve son
objet:
\begin{equation*}
...
minimum\index{pile!filtrage!coût minimum} de \(\C{\fun{sfst}}{n,j}\),
...
maximum\index{pile!filtrage!coût maximum}
...
\end{equation*}
par l'équation~\eqref{eq_sum_k} \vpageref{eq_sum_k}.

Notons que la règle~\(\kappa\) implique la création d'un
n{\oe}ud~(\texttt{|}), que nous appelons \emph{n{\oe}ud
  d'empilage}\index{n{\oe}ud d'empilage}, comme on peut le voir à la
\fig~\vref{fig_sfst_dag}.
\begin{figure}
\centering
...
la \fig~\ref{fig_sfst0}.\index{sfst@\fun{sfst/4}}
\begin{figure}[b]
\begin{equation*}
\boxed{%
...
\fig~\vref{fig_sfst1}\index{sfst1@\fun{sfst\(_1\)/1}}
\begin{figure}
\begin{equation*}
\boxed{%
\begin{array}{r@{\;}l@{\;}l}
...
\vpageref{eq_rev0}\index{rev0@\fun{rev\(_0\)/1}}, et à partir de
...
maximum:\index{pile!filtrage!coût minimum}\index{pile!filtrage!coût
  maximum}
\begin{equation*}
...
Considérons la \fig~\ref{fig_slst},
\begin{figure}[b]
\begin{equation*}
\boxed{%
\begin{array}{@{}r@{\;}l@{\;}lr@{\;}l@{\;}l@{}}
...
\label{fig_slst}\index{pile!filtrage!dernière occurrence}}
\end{figure}
...
Considérons l'évaluation à \fig~\vref{fig_slst_27071}.
\begin{figure}
\begin{equation*}
\boxed{%
\begin{array}{r@{\;}l@{\;}l}
...
\caption{\(\fun{slst}([2,7,0,7,1],7) \twoheadrightarrow
  [2,7,0,1]\)\label{fig_slst_27071}\index{pile!filtrage!exemple}}
\end{figure}
...
\end{equation*}
Nous en déduisons que le coût minimum\index{pile!filtrage!coût
  minimum} se produit quand \(l-f+1 = p\), c'est-à-dire quand toutes
les occurrences sont consécutives, par conséquent
\(\B{\fun{slst}}{n,p} = n + p\)\index{slst@$\B{\fun{slst}}{n}$}. Le
coût maximum\index{pile!filtrage!coût maximum} arrive quand~\({f=0}\)
et~\({l=n-1}\), c'est-à-dire quand il y a au moins deux occurrences
de~\(x\), une au sommet et une autre au fond: \(\W{\fun{slst}}{n} =
2n\)\index{slst@$\W{\fun{slst}}{n}$}. Nous pouvons vérifier que
lorsque la pile est entièrement constituée de~\(x\), les extremums
concourent à la valeur~\(2n\). Le coût moyen\index{pile!filtrage!coût
  moyen} \index{slst@$\M{\fun{slst}}{n}$} quand \(x\)~est présent
...
\vpageref{eq_sum_k}, et la somme des carrés successifs est calculée de
la façon suivante.
...
\begin{enumerate}
  ...
  \item Montrez que, dans le pire des cas à déterminer,
    \(\fun{slst}_0(s,x)\) créé \(3n\)~n{\oe}uds inutiles si~\(s\)
